...
from panda_gym.envs.tasks.multi_reward_stack import MultiRewardStack
from panda_gym.envs.tasks.multi_reward_pick_and_place import MultiRewardPickAndPlace
from panda_gym.envs.tasks.multi_reward_pick_and_place_sphere import MultiRewardPickAndPlaceSphere
from panda_gym.envs.tasks.multi_reward_pick_and_place_obstacle import MultiRewardPickAndPlaceObstacle
...
class PandaMultiRewardStackEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        weights_shape = observation["weights"].shape
        self.observation_space = spaces.Dict(
            dict(
                ...
                weights=spaces.Box(0.0, 1.0, shape=weights_shape, dtype=np.float32)
            )
        )
        
    def _get_obs(self) -> Dict[str, np.ndarray]:
        observation = super()._get_obs()
        observation["weights"] = np.ones(self.reward_dim) * np.nan
        return observation


class PandaMultiRewardPickAndPlaceEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = MultiRewardPickAndPlace(sim, get_ee_position=robot.get_ee_position, reward_type=reward_type)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaMultiRewardPickAndPlaceSphereEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = MultiRewardPickAndPlaceSphere(sim, get_ee_position=robot.get_ee_position, reward_type=reward_type)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaMultiRewardPickAndPlaceObstacleEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = MultiRewardPickAndPlaceObstacle(sim, get_ee_position=robot.get_ee_position, reward_type=reward_type)
        super().__init__(
            robot,
            task,
            render_width=render_width,
