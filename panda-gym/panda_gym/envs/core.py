...
class Task(ABC):
    ...
    def set_goal(self, goal):
        assert goal.shape == self.goal.shape
        self.goal = goal
...
class RobotTaskEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        terminated = False
        truncated = False
        info = {"is_success": bool(self.task.is_success(observation["achieved_goal"], self.task.get_goal()))}
        reward = self.task.compute_reward(observation["achieved_goal"], self.task.get_goal(), info).astype(float)
